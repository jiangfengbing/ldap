...
func HandleBindRequest(req *ber.Packet, fns map[string]Binder, c *Context) (resultCode LDAPResultCode) {
	defer func() {
		if r := recover(); r != nil {
	...
	case LDAPBindAuthSimple:
		if len(req.Children) == 3 {
			...
			resultCode, err := fns[fn].Bind(bindDN, bindAuth.Data.String(), c)
			if err != nil {
